...
model.getClient = function (clientId, clientSecret, callback) {
	global.sqlPool.getConnection(function (err, connection) {
		let sqlGetClient = "select * from client where client_id = '" + clientId + "' and client_secret = '" + clientSecret + "';";
...
model.saveRefreshToken = function (refreshToken, clientId, expires, userId, callback) {
    global.sqlPool.getConnection(function (err, connection) {
		...
		let sqlAddToken = "insert into token(token, tokentype, clientid, userid, expires) values('" + refreshToken + "', 'refresh', '" + clientId + "', " + userId.id + ", '" + js_yyyy_mm_dd_hh_mm_ss(expires) + "');";
		connection.query(sqlClearToken + sqlAddToken, function (error, results, fields) {
            connection.release();
			if (!error) {
                callback(false);
